...
from utils import clear_objs_and_retain_memory
...
def log_validation(controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                    validation_prompt, control_image=validation_image, num_inference_steps=20, controlnet_conditioning_scale=0.8,
                    guidance_scale=7.0, generator=generator, height=args.resolution, width=args.resolution
                ).images[0]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.05, help="use to adjust the importance of cost loss",
    )
    parser.add_argument(
        "--sparse", type=float, default=0.5, help="use to control the ratio of keep adapter",
    )
    parser.add_argument(
        "--thred", type=float, default=0.5, help="use as the threshold in gumel-sigmoid",
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = SD3ControlNetModel.from_transformer(transformer, num_extra_conditioning_channels=0, num_layers=23)
    ...
    with accelerator.main_process_first():
        import datasets
        train_dataset = datasets.load_from_disk("/dataset/llava558k_cache_"+str(args.resolution))
        train_dataset = train_dataset["train"]
        print("load latents and embeddings from the disk successfully")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                model_input = batch["pixel_latents"].to(dtype=weight_dtype)
                ...
                controlnet_image = batch["control_latents"].to(dtype=weight_dtype)

                control_block_res_samples, mask = controlnet(
