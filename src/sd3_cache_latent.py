...
def main(args):
    ...
    def compute_embeddings(batch, text_encoders, tokenizers, vae):
        ...
        pixel_latents = pixel_latents_tmp.to(dtype=weight_dtype)
        ...
        return {"prompt_embeds": prompt_embeds, "pooled_prompt_embeds": pooled_prompt_embeds, "pixel_latents": pixel_latents, "control_latents": control_latents}
    ...
    train_dataset = train_dataset.remove_columns(['image', 'conditioning_image', 'text'])

    from datasets import DatasetDict
    DatasetDict({'train': train_dataset.with_format(None)}).save_to_disk(args.save_local_adr)
    print("save latents and embeddings to the disk successfully")
