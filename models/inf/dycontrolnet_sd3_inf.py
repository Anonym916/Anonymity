...
from models.dycontrolnet import BaseOutput, zero_module
from module.gatenetdit import controller
...
class SD3ControlNetModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        for _ in range(len(self.transformer_blocks)):
            gate_block = controller()
            self.gate_blocks.append(gate_block)
    ...
    def forward(
        ...
        for block, gate in zip(self.transformer_blocks, self.gate_blocks):
            mask = gate(hidden_states)
            # old_hidden_states = hidden_states
            # old_encoder_hidden_states = encoder_hidden_states
            ...
            mask_samples = mask_samples + (mask,)

            # hidden_states = mul(hidden_states, mask) + mul(old_hidden_states, (1 - mask))
            # encoder_hidden_states = mul(encoder_hidden_states, mask) + mul(old_encoder_hidden_states, (1 - mask))

        controlnet_block_res_samples = ()
        for block_res_sample, controlnet_block, mask in zip(block_res_samples, self.controlnet_blocks, mask_samples):
            if mask[0] == 1:
                block_res_sample = controlnet_block(block_res_sample)
                # block_res_sample = mul(block_res_sample, mask)
                controlnet_block_res_samples = controlnet_block_res_samples + (block_res_sample,)
            else:
                controlnet_block_res_samples = controlnet_block_res_samples + (0,)
        ...
        if return_mask:
            if not return_dict:
                return (controlnet_block_res_samples, mask_samples)
            else:
